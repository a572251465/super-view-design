...
const linuxContent: IListTemplate = {
    ...
    content: [
        ...
        {detail: '$ cp <源文件> <目标地址>', notes: '将源文件复制到指定目录下 -r 可以附带文件夹'},
        {detail: '$ mv <源文件> <目标文件>', notes: '移动文件 -- 同目录下进行改名, 不同目录下直接剪切'},
        {detail: '$ ln -s <源文件> <目标>', notes: '创建链接'},
        {detail: '$ whereis <命令名称>', notes: '搜索命令所在目录, 以及帮助文档'},
        {detail: '$ find <搜索范围> <搜索文件>', notes: '指定目录下搜索文件'},
        {detail: '$ find / -size +0k', notes: '按大小进行搜索 -- 例如: -8k 小8k 8K = 等于8k +8k = 大于8k'},
        ...
        {detail: '$ chmod <权限> <文件>', notes: '修改权限 r => 4 w => 2 x => 1 777 表示最高权限'}
    ]
}
